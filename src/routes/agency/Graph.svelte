<script lang="ts">
	...
	import type { ForceGraphGenericInstance, LinkObject, NodeObject } from 'force-graph';
	import ForceGraph from 'force-graph';
	import { onDestroy, onMount } from 'svelte';
	import type { agencyResponseData } from './+page.server';

	export let data: agencyResponseData;
	...
	const NODE_AGENCY_COLOR = '#3b82f6';
	const NODE_RETIRED_COLOR = '#713e12';
	const NODE_INACTIVE_COLOR = 'rgba(0,0,0,0.1)';

	const LINK_COLOR = 'rgba(59, 130, 246, 0.3)';
	const LINK_ACTIVE_COLOR = 'rgba(59, 130, 246, 1)';
	...
	const getNodeColor = (node: GraphNode | any): string => {
		if (hoverNodeID !== 0) {
			if ([...hoverNodeIDs, hoverNodeID].includes(node.id)) {
				return node.id < 0 ? NODE_AGENCY_COLOR : node.hasRetired ? NODE_RETIRED_COLOR : NODE_COLOR;
			}
