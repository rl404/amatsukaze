<script lang="ts">
	...
	const illustratorData = Object.entries(
		data.reduce((res: { [name: string]: number }, curr) => {
			curr.character_designers?.forEach((a) => {
				if (!res[a]) res[a] = 0;
				res[a]++;
			});
			return res;
	...
	const onClick = (e: any) => {
		...
		modalData = data.filter((d) => d.character_designers?.find((a) => a === chartData.group));
		modal.toggleModal();
	};
</script>

<div class="text-center font-bold">Character Designer</div>
